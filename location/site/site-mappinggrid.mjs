...
import * as hnd from  './site-mappinggrid-hnd.mjs'

const tbl_list = $('#pnl_editmappinggrid-tbl_list');
const txt_title = $('#pnl_editmappinggrid-title');
const pnl_control = $('.pnl_editmappinggrid-control');
const btn_removechecked  = $('#pnl_editmappinggrid-removechecked');
const btn_addrow = $('#pnl_editmappinggrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/mapping-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editmappingform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editmappingform', () => {
		$ui.getPages().ITEMS['pnl_editmappingform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/mapping-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editmappingform', ()=>{
		$ui.getPages().ITEMS['pnl_editmappingform'].handler.createnew(header_data)
	})	
}