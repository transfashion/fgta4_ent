...
import * as hnd from  './site-eventgrid-hnd.mjs'

const tbl_list = $('#pnl_editeventgrid-tbl_list');
const txt_title = $('#pnl_editeventgrid-title');
const pnl_control = $('.pnl_editeventgrid-control');
const btn_removechecked  = $('#pnl_editeventgrid-removechecked');
const btn_addrow = $('#pnl_editeventgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/event-list`
		options.criteria['id'] = data.site_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editeventform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editeventgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editeventgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editeventform', () => {
		$ui.getPages().ITEMS['pnl_editeventform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/event-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editeventform', ()=>{
		$ui.getPages().ITEMS['pnl_editeventform'].handler.createnew(header_data)
	})	
}