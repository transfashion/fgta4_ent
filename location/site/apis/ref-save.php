...
/**
 * ent/location/site/apis/ref-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel ref site (mst_siteref)
 *
...
$API = new class extends siteBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_siteref';
		$primarykey = 'siteref_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\site_refHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new site_refHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"siteref_id" => $obj->siteref_id
				];

				$criteriaValues = [
					"siteref_id" => " siteref_id = :siteref_id "
				];
