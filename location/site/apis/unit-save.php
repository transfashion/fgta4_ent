...
if (is_file(__DIR__ .'/data-unit-handler.php')) {
	require_once __DIR__ .'/data-unit-handler.php';
}
...
/**
 * ent/location/site/apis/unit-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel unit site (mst_siteunit)
 *
...
$API = new class extends siteBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_siteunit';
		$primarykey = 'siteunit_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\site_unitHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new site_unitHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->siteunit_id = strtoupper($obj->siteunit_id);
			...
			try {
				...
				// result
				$options->criteria = [
					"siteunit_id" => $obj->siteunit_id
				];

				$criteriaValues = [
					"siteunit_id" => " siteunit_id = :siteunit_id "
				];
