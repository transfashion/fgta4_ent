...
if (is_file(__DIR__ .'/data-event-handler.php')) {
	require_once __DIR__ .'/data-event-handler.php';
}
...
/**
 * ent/location/site/apis/event-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel event site (mst_siteevent)
 *
...
$API = new class extends siteBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_siteevent';
		$primarykey = 'siteevent_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\site_eventHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new site_eventHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->siteevent_date = (\DateTime::createFromFormat('d/m/Y',$obj->siteevent_date))->format('Y-m-d');

			$obj->siteevent_id = strtoupper($obj->siteevent_id);
			$obj->siteevent_name = strtoupper($obj->siteevent_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"siteevent_id" => $obj->siteevent_id
				];

				$criteriaValues = [
					"siteevent_id" => " siteevent_id = :siteevent_id "
				];
				...
				$sqlFieldList = [
					'siteevent_id' => 'A.`siteevent_id`', 'siteevent_name' => 'A.`siteevent_name`', 'siteevent_date' => 'A.`siteevent_date`', 'siteevent_enabling' => 'A.`siteevent_enabling`',
					'siteevent_disabling' => 'A.`siteevent_disabling`', 'site_id' => 'A.`site_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_siteevent A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'siteevent_date' => date("d/m/Y", strtotime($row['siteevent_date'])),
