...
if (is_file(__DIR__ .'/data-mapping-handler.php')) {
	require_once __DIR__ .'/data-mapping-handler.php';
}
...
/**
 * ent/location/site/apis/mapping-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel mapping site (mst_sitemapping)
 *
...
$API = new class extends siteBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_sitemapping';
		$primarykey = 'sitemapping_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\site_mappingHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new site_mappingHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->sitemapping_id = strtoupper($obj->sitemapping_id);
			$obj->sitemapping_mapfrom = strtoupper($obj->sitemapping_mapfrom);
			...
			try {
				...
				// result
				$options->criteria = [
					"sitemapping_id" => $obj->sitemapping_id
				];

				$criteriaValues = [
					"sitemapping_id" => " sitemapping_id = :sitemapping_id "
				];
				...
				$sqlFieldList = [
					'sitemapping_id' => 'A.`sitemapping_id`', 'sitemapping_mapfrom' => 'A.`sitemapping_mapfrom`', 'unit_id' => 'A.`unit_id`', 'dept_id' => 'A.`dept_id`',
					'site_id' => 'A.`site_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_sitemapping A";
				$sqlWhere = $where->sql;
