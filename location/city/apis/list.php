...
/**
 * ent/location/city/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header city (mst_city)
 ...
 * tanggal 14/12/2022
 */
$API = new class extends cityBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\city_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new city_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlFieldList = [
				'city_id' => 'A.`city_id`', 'city_name' => 'A.`city_name`', 'city_isdisabled' => 'A.`city_isdisabled`', 'prov_id' => 'A.`prov_id`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_city A";
			$sqlWhere = $where->sql;
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('prov_name', 'prov_id', $record, 'mst_prov', 'prov_name', 'prov_id');
