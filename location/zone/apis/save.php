...
/**
 * ent/location/zone/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header zone (mst_zone)
 *
 ...
 * tanggal 14/12/2022
 */
$API = new class extends zoneBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\zone_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new zone_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->zone_descr=='') { $obj->zone_descr = '--NULL--'; }
			...
			try {
				...
				$sqlFieldList = [
					'zone_id' => 'A.`zone_id`', 'zone_name' => 'A.`zone_name`', 'zone_isdisabled' => 'A.`zone_isdisabled`', 'zone_descr' => 'A.`zone_descr`',
					'auth_id' => 'A.`auth_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_zone A";
				$sqlWhere = $where->sql;
