...
/**
 * ent/location/country/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header country (mst_country)
 ...
 * tanggal 27/08/2024
 */
$API = new class extends countryBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\country_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new country_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlFieldList = [
				'country_id' => 'A.`country_id`', 'country_name' => 'A.`country_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_country A";
			$sqlWhere = $where->sql;
