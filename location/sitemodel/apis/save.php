...
/**
 * ent/location/sitemodel/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header sitemodel (mst_sitemodel)
 *
 ...
 * tanggal 27/08/2024
 */
$API = new class extends sitemodelBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\sitemodel_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new sitemodel_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->sitemodel_descr=='') { $obj->sitemodel_descr = '--NULL--'; }
			...
			try {
				...
				$sqlFieldList = [
					'sitemodel_id' => 'A.`sitemodel_id`', 'sitemodel_name' => 'A.`sitemodel_name`', 'sitemodel_descr' => 'A.`sitemodel_descr`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_sitemodel A";
				$sqlWhere = $where->sql;
