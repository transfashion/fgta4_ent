...
/**
 * ent/location/prov/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header prov (mst_prov)
 ...
 * tanggal 14/12/2022
 */
$API = new class extends provBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\prov_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new prov_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlFieldList = [
				'prov_id' => 'A.`prov_id`', 'prov_name' => 'A.`prov_name`', 'prov_capital' => 'A.`prov_capital`', 'area_id' => 'A.`area_id`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_prov A";
			$sqlWhere = $where->sql;
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('area_name', 'area_id', $record, 'mst_area', 'area_name', 'area_id');
