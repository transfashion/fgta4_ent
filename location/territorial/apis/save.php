...
/**
 * ent/location/territorial/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header territorial (mst_territorial)
 *
 ...
 * tanggal 27/08/2024
 */
$API = new class extends territorialBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_territorial';
		$primarykey = 'territorial_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\territorial_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new territorial_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->territorial_id = strtoupper($obj->territorial_id);
			$obj->territorial_name = strtoupper($obj->territorial_name);


			if ($obj->territorial_descr=='') { $obj->territorial_descr = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"territorial_id" => $obj->territorial_id
				];

				$criteriaValues = [
					"territorial_id" => " territorial_id = :territorial_id "
				];
				...
				$sqlFieldList = [
					'territorial_id' => 'A.`territorial_id`', 'territorial_name' => 'A.`territorial_name`', 'territorial_descr' => 'A.`territorial_descr`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_territorial A";
				$sqlWhere = $where->sql;
