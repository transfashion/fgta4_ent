...
/**
 * ent/items/itemprop/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header itemprop (mst_itemprop)
 *
 ...
 * tanggal 28/08/2024
 */
$API = new class extends itempropBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\itemprop_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new itemprop_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->itemprop_name = strtoupper($obj->itemprop_name);
			...
			try {
				...
				$sqlFieldList = [
					'itemprop_id' => 'A.`itemprop_id`', 'itemprop_name' => 'A.`itemprop_name`', 'itemprop_descr' => 'A.`itemprop_descr`', 'itemprop_tags' => 'A.`itemprop_tags`',
