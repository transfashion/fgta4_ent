...
/**
 * ent/items/itemmodel/apis/prop-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel prop itemmodel (mst_itemmodelprop)
 *
...
$API = new class extends itemmodelBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_itemmodelprop';
		$primarykey = 'itemmodelprop_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\itemmodel_propHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new itemmodel_propHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'itemmodelprop_id', 'prop_id', 'itemmodelprop_order', 'itemmodel_id'
			];
			...
			$obj->itemmodelprop_id = strtoupper($obj->itemmodelprop_id);
			...
			try {
				...
				$header_table = 'mst_itemmodel';
				$header_primarykey = 'itemmodel_id';
				$detil_primarykey = 'itemmodel_id';
				...
				// result
				$options->criteria = [
					"itemmodelprop_id" => $obj->itemmodelprop_id
				];

				$criteriaValues = [
					"itemmodelprop_id" => " itemmodelprop_id = :itemmodelprop_id "
				];
				...
				$sqlFieldList = [
					'itemmodelprop_id' => 'A.`itemmodelprop_id`', 'prop_id' => 'A.`prop_id`', 'itemmodelprop_order' => 'A.`itemmodelprop_order`', 'itemmodel_id' => 'A.`itemmodel_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_itemmodelprop A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'prop_name' => \FGTA4\utils\SqlUtility::Lookup($record['prop_id'], $this->db, 'mst_prop', 'prop_id', 'prop_name'),
