...
/**
 * ent/items/itemmodel/apis/prop-list.php
 *
 * ==============
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel prop itemmodel (mst_itemmodelprop)
...
$API = new class extends itemmodelBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new itemmodel_propHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('prop_name', 'prop_id', $record, 'mst_prop', 'prop_name', 'prop_id');
