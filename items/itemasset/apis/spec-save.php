...
if (is_file(__DIR__ .'/data-spec-handler.php')) {
	require_once __DIR__ .'/data-spec-handler.php';
}
...
/**
 * ent/items/itemasset/apis/spec-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel spec itemasset (mst_itemassetspec)
 *
...
$API = new class extends itemassetBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\itemasset_specHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new itemasset_specHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'itemassetspec_id', 'itemassetspec_value', 'itemspec_id', 'itemspec_isnumber',
					'itemasset_id'
			];
			...
			try {
				...
				// result
				$options->criteria = [
					"itemassetspec_id" => $obj->itemassetspec_id
				];
				...
				$sqlFieldList = [
					'itemassetspec_id' => 'A.`itemassetspec_id`', 'itemassetspec_value' => 'A.`itemassetspec_value`', 'itemspec_id' => 'A.`itemspec_id`', 'itemspec_isnumber' => 'A.`itemspec_isnumber`',
					'itemasset_id' => 'A.`itemasset_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_itemassetspec A";
				$sqlWhere = $where->sql;
