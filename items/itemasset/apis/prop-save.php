...
/**
 * ent/items/itemasset/apis/prop-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel prop itemasset (mst_itemassetprop)
 *
...
$API = new class extends itemassetBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_itemassetprop';
		$primarykey = 'itemassetprop_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\itemasset_propHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new itemasset_propHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'itemassetprop_id', 'prop_id', 'itemassetprop_value', 'itemassetprop_order',
					'itemasset_id'
			];
			...
			$obj->itemassetprop_id = strtoupper($obj->itemassetprop_id);
			...
			try {
				...
				// result
				$options->criteria = [
					"itemassetprop_id" => $obj->itemassetprop_id
				];

				$criteriaValues = [
					"itemassetprop_id" => " itemassetprop_id = :itemassetprop_id "
				];
				...
				$sqlFieldList = [
					'itemassetprop_id' => 'A.`itemassetprop_id`', 'prop_id' => 'A.`prop_id`', 'itemassetprop_value' => 'A.`itemassetprop_value`', 'itemassetprop_order' => 'A.`itemassetprop_order`',
					'itemasset_id' => 'A.`itemasset_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_itemassetprop A";
				$sqlWhere = $where->sql;
				...
				$dataresponse = array_merge($record, [
					...
					'prop_name' => \FGTA4\utils\SqlUtility::Lookup($record['prop_id'], $this->db, 'mst_itemprop', 'prop_id', 'prop_name'),
