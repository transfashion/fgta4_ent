...
import * as hnd from  './itemstockperiode-costgrid-hnd.mjs'

const tbl_list = $('#pnl_editcostgrid-tbl_list');
const txt_title = $('#pnl_editcostgrid-title');
const pnl_control = $('.pnl_editcostgrid-control');
const btn_removechecked  = $('#pnl_editcostgrid-removechecked');
const btn_addrow = $('#pnl_editcostgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/cost-list`
		options.criteria['id'] = data.itemstockperiode_id
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editcostform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editcostgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editcostgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editcostform', () => {
		$ui.getPages().ITEMS['pnl_editcostform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/cost-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editcostform', ()=>{
		$ui.getPages().ITEMS['pnl_editcostform'].handler.createnew(header_data)
	})	
}