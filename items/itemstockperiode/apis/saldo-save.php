...
/**
 * ent/items/itemstockperiode/apis/saldo-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel saldo itemstockperiode (mst_itemstocksaldo)
 *
...
$API = new class extends itemstockperiodeBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new itemstockperiode_saldoHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				// result
				$options->criteria = [
					"itemstocksaldo_id" => $obj->itemstocksaldo_id
				];
