...
/**
 * ent/items/item/apis/prop-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel prop item (mst_itemprop)
 *
...
$API = new class extends itemBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_itemprop';
		$primarykey = 'itemprop_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\item_propHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new item_propHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'itemprop_id', 'prop_id', 'itemmodelprop_order', 'itemmodel_id'
			];
			...
			$obj->itemprop_id = strtoupper($obj->itemprop_id);
			...
			try {
				...
				$header_table = 'mst_item';
				$header_primarykey = 'item_id';
				$detil_primarykey = 'item_id';
				...
				// result
				$options->criteria = [
					"itemprop_id" => $obj->itemprop_id
				];

				$criteriaValues = [
					"itemprop_id" => " itemprop_id = :itemprop_id "
				];
				...
				$sqlFieldList = [
					'itemprop_id' => 'A.`itemprop_id`', 'prop_id' => 'A.`prop_id`', 'itemmodelprop_order' => 'A.`itemmodelprop_order`', 'itemmodel_id' => 'A.`itemmodel_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_itemprop A";
				$sqlWhere = $where->sql;
