...
/**
 * ent/items/prop/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header prop (mst_prop)
 ...
 * tanggal 28/08/2024
 */
$API = new class extends propBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\prop_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new prop_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.prop_id LIKE CONCAT('%', :search, '%') OR A.prop_name LIKE CONCAT('%', :search, '%') OR A.prop_tags LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'prop_id' => 'A.`prop_id`', 'prop_name' => 'A.`prop_name`', 'prop_descr' => 'A.`prop_descr`', 'prop_tags' => 'A.`prop_tags`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_prop A";
			$sqlWhere = $where->sql;
