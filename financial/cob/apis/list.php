...
/**
 * ent/financial/cob/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header cob (mst_cob)
 ...
 * tanggal 30/08/2024
 */
$API = new class extends cobBase {

	public function execute($options) {

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\cob_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new cob_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$criteriaValues = [
				"search" => " A.cob_id LIKE CONCAT('%', :search, '%') OR A.cob_name LIKE CONCAT('%', :search, '%') "
			];
			...
			$sqlFieldList = [
				'cob_id' => 'A.`cob_id`', 'cobgroup_id' => 'A.`cobgroup_id`', 'cob_name' => 'A.`cob_name`', 'cob_nameshort' => 'A.`cob_nameshort`',
				'cob_descr' => 'A.`cob_descr`', 'cob_isdisabled' => 'A.`cob_isdisabled`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_cob A";
			$sqlWhere = $where->sql;
			...
			foreach ($rows as $row) {
				...
				$record = array_merge($record, [
					...
					//'tanggal' => date("d/m/y", strtotime($record['tanggal'])),
				 	//'tambahan' => 'dta'
					'cobgroup_name' => \FGTA4\utils\SqlUtility::Lookup($record['cobgroup_id'], $this->db, 'mst_cobgroup', 'cobgroup_id', 'cobgroup_name'),
				...
				*/
				...
				$this->addFields('cobgroup_name', 'cobgroup_id', $record, 'mst_cobgroup', 'cobgroup_name', 'cobgroup_id');
