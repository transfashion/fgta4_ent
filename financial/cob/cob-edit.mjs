...
import * as hnd from  './cob-edit-hnd.mjs'
...
const obj = {
	txt_cob_id: $('#pnl_edit-txt_cob_id'),
	cbo_cobgroup_id: $('#pnl_edit-cbo_cobgroup_id'),
	txt_cob_name: $('#pnl_edit-txt_cob_name'),
	txt_cob_nameshort: $('#pnl_edit-txt_cob_nameshort'),
	txt_cob_descr: $('#pnl_edit-txt_cob_descr'),
	chk_cob_isdisabled: $('#pnl_edit-chk_cob_isdisabled')
}
...
export async function init(opt) {
	...
	obj.cbo_cobgroup_id.name = 'pnl_edit-cbo_cobgroup_id'		
	new fgta4slideselect(obj.cbo_cobgroup_id, {
		title: 'Pilih cobgroup_id',
		returnpage: this_page_id,
		api: $ui.apis.load_cobgroup_id,
		fieldValue: 'cobgroup_id',
		fieldDisplay: 'cobgroup_name',
		fields: [
		...
		OnDataLoading: (criteria, options) => {
			...
			if (typeof hnd.cbo_cobgroup_id_dataloading === 'function') {
				hnd.cbo_cobgroup_id_dataloading(criteria, options);
			}						
		},					
		OnSelected: (value, display, record, args) => {
			if (value!=args.PreviousValue ) {
				if (typeof hnd.cbo_cobgroup_id_selected === 'function') {
					hnd.cbo_cobgroup_id_selected(value, display, record, args);
				}
			}
		},
...
export function open(data, rowid, viewmode=true, fn_callback) {
	...
	var fn_dataopened = async (result, options) => {
		...
		*/
		...
		form
			.fill(record)
			.setValue(obj.cbo_cobgroup_id, record.cobgroup_id, record.cobgroup_name)
...
export function createnew() {
	...
	form.createnew(async (data, options)=>{
		...
		data.cob_isdisabled = '0'

		data.cobgroup_id = '0'
		data.cobgroup_name = '-- PILIH --'
