...
/**
 * ent/organisation/unit/apis/itemclass-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel itemclass unit (mst_unititemclass)
 *
...
$API = new class extends unitBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_unititemclass';
		$primarykey = 'unititemclass_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\unit_itemclassHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new unit_itemclassHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'unititemclass_id', 'itemclass_id', 'unit_id'
			];
			...
			try {
				...
				// result
				$options->criteria = [
					"unititemclass_id" => $obj->unititemclass_id
				];

				$criteriaValues = [
					"unititemclass_id" => " unititemclass_id = :unititemclass_id "
				];
				...
				$sqlFieldList = [
					'unititemclass_id' => 'A.`unititemclass_id`', 'itemclass_id' => 'A.`itemclass_id`', 'unit_id' => 'A.`unit_id`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_unititemclass A";
				$sqlWhere = $where->sql;
