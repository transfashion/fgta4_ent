...
/**
 * ent/organisation/unit/apis/ref-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel ref unit (mst_unitref)
 *
...
$API = new class extends unitBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_unitref';
		$primarykey = 'unitref_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\unit_refHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new unit_refHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'unitref_id', 'interface_id', 'unitref_name', 'unitref_code',
					'unitref_otherdata', 'unitref_notes', 'unit_id'
			];
			...
			$obj->unitref_code = strtoupper($obj->unitref_code);


			if ($obj->unitref_otherdata=='') { $obj->unitref_otherdata = '--NULL--'; }
			if ($obj->unitref_notes=='') { $obj->unitref_notes = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"unitref_id" => $obj->unitref_id
				];

				$criteriaValues = [
					"unitref_id" => " unitref_id = :unitref_id "
				];
