...
import * as hnd from  './docs-authgrid-hnd.mjs'
...
export function OpenDetil(data) {
	...
	var fn_listloaded = async (result, options) => {
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editauthgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editauthgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
