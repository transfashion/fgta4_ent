...
/**
 * ent/organisation/deptgroup/apis/list.php
 *
 * ========
 * DataList
 * ========
 * Menampilkan data-data pada tabel header deptgroup (mst_deptgroup)
...
$API = new class extends deptgroupBase {
	...
	public function execute($options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\deptgroup_headerHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new deptgroup_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlFieldList = [
				...
				'deptgroup_parent' => 'A.`deptgroup_parent`', 'deptgroup_pathid' => 'A.`deptgroup_pathid`', 'deptgroup_path' => 'A.`deptgroup_path`', 'deptgroup_level' => 'A.`deptgroup_level`',
				'deptgroup_isexselect' => 'A.`deptgroup_isexselect`', 'depttype_id' => 'A.`depttype_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
