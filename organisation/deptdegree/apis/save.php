...
/**
 * ent/organisation/deptdegree/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header deptdegree (mst_deptdegree)
 *
 ...
 * tanggal 24/08/2024
 */
$API = new class extends deptdegreeBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_deptdegree';
		$primarykey = 'deptdegree_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\deptdegree_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new deptdegree_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->deptdegree_name = strtoupper($obj->deptdegree_name);


			if ($obj->deptdegree_descr=='') { $obj->deptdegree_descr = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"deptdegree_id" => $obj->deptdegree_id
				];

				$criteriaValues = [
					"deptdegree_id" => " deptdegree_id = :deptdegree_id "
				];
