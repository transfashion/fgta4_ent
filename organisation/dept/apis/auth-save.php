...
if (is_file(__DIR__ .'/data-auth-handler.php')) {
	require_once __DIR__ .'/data-auth-handler.php';
}
...
/**
 * ent/organisation/dept/apis/auth-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel auth dept (mst_deptauth)
 *
...
$API = new class extends deptBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\dept_authHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new dept_authHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'deptauth_id', 'auth_id', 'authlevel_id', 'dept_id'
			];
			...
			try {
				...
				$sqlFieldList = [
					'deptauth_id' => 'A.`deptauth_id`', 'auth_id' => 'A.`auth_id`', 'authlevel_id' => 'A.`authlevel_id`', 'dept_id' => 'A.`dept_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_deptauth A";
				$sqlWhere = $where->sql;
