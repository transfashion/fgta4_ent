...
$API = new class extends deptBase {
	...
	public function execute($options) {
		...
		try {
			...
			\FGTA4\utils\SqlUtility::setDefaultCriteria($options->criteria, 'dept_isparent', '1');
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				...
				[
					...
					"dept_isdisabled" => " A.dept_isdisabled = :dept_isdisabled ",
				]
			);
			...
			$stmt = $this->db->prepare("
				select 
				    A.dept_id as dept_parent, A.dept_name as dept_parent_name
				  , A.depttype_id
				from mst_dept A
			" 
			. $where->sql 
			. " ORDER BY dept_parent_name "
			. $limit);
			$stmt->execute($where->params);
