...
/**
 * ent/organisation/auth/apis/delegate-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel delegate auth (mst_authdelegate)
 *
...
$API = new class extends authBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new auth_delegateHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'authdelegate_id', 'authdelegate_portion', 'empl_id', 'auth_id'
			];
			...
			try {
				...
				// result
				$options->criteria = [
					"authdelegate_id" => $obj->authdelegate_id
				];
				...
				$sqlFieldList = [
					'authdelegate_id' => 'A.`authdelegate_id`', 'authdelegate_portion' => 'A.`authdelegate_portion`', 'empl_id' => 'A.`empl_id`', 'auth_id' => 'A.`auth_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_authdelegate A";
				$sqlWhere = $where->sql;
