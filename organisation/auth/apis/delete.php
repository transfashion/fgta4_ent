...
/**
 * ent/organisation/auth/apis/delete.php
 *
 * ======
 * Delete
 * ======
 * Menghapus satu baris data/record berdasarkan PrimaryKey
 * pada tabel header auth (mst_auth)
 *
...
$API = new class extends authBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\auth_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new auth_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$tabletodelete = ['mst_authdelegate'];
