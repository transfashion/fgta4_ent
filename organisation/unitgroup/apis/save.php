...
/**
 * ent/organisation/unitgroup/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header unitgroup (mst_unitgroup)
 *
 ...
 * tanggal 25/08/2024
 */
$API = new class extends unitgroupBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\unitgroup_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new unitgroup_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			try {
				...
				$sqlFieldList = [
					'unitgroup_id' => 'A.`unitgroup_id`', 'unitgroup_name' => 'A.`unitgroup_name`', 'unitgroup_descr' => 'A.`unitgroup_descr`', 'unitgroup_isdisabled' => 'A.`unitgroup_isdisabled`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_unitgroup A";
				$sqlWhere = $where->sql;
