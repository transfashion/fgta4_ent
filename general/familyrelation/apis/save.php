...
/**
 * ent/general/familyrelation/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header familyrelation (mst_familyrelation)
 *
 ...
 * tanggal 26/08/2024
 */
$API = new class extends familyrelationBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_familyrelation';
		$primarykey = 'familyrelation_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\familyrelation_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new familyrelation_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->familyrelation_id = strtoupper($obj->familyrelation_id);
			$obj->familyrelation_name = strtoupper($obj->familyrelation_name);
			...
			try {
				...
				// result
				$options->criteria = [
					"familyrelation_id" => $obj->familyrelation_id
				];

				$criteriaValues = [
					"familyrelation_id" => " familyrelation_id = :familyrelation_id "
				];
				...
				$sqlFieldList = [
					'familyrelation_id' => 'A.`familyrelation_id`', 'familyrelation_name' => 'A.`familyrelation_name`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_familyrelation A";
				$sqlWhere = $where->sql;
