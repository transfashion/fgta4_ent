 ...
 * dari tabel header contacttype (mst_contacttype)
 *
...
$API = new class extends contacttypeBase {
	...
	public function execute($data, $options) {
		...
		$handlerclassname = "\\FGTA4\\apis\\contacttype_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new contacttype_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			if ($obj->contacttype_descr=='') { $obj->contacttype_descr = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"contacttype_id" => $obj->contacttype_id
				];
				...
				$sqlFieldList = [
					'contacttype_id' => 'A.`contacttype_id`', 'contacttype_name' => 'A.`contacttype_name`', 'contacttype_descr' => 'A.`contacttype_descr`', '_createby' => 'A.`_createby`',
