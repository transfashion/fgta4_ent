...
class DataSave extends WebAPI {
	...
	public function execute($data, $options) {
		...
		$tablename = 'con_weekdt';
		$primarykey = 'weekdt_id';
		...
		try {
			...
			$obj->weekdt_date = (\DateTime::createFromFormat('d/m/Y',$obj->weekdt_date))->format('Y-m-d');
			...
			try {
				...
				$header_table = 'con_week';
				$header_primarykey = 'week_id';
			...
			$sql = \FGTA4\utils\SqlUtility::Select($tablename , [
				$primarykey,  'weekdt_id', 'weekdt_date', 'weekdt_dayname', 'weekdt_daynum', 'weekdt_year', 'weekdt_month', 'week_id', '_createby', '_createdate', '_modifyby', '_modifydate', '_createby', '_createdate', '_modifyby', '_modifydate'
			], $where->sql);
			...
			$result->dataresponse = (object) array_merge($dataresponse, [
				...
				'weekdt_date' => date("d/m/Y", strtotime($row['weekdt_date'])),
