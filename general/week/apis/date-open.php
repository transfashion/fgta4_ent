	...
	public function execute($options) {
		...
		try {
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				$options->criteria,
				[
					"weekdt_id" => " weekdt_id = :weekdt_id "
				]
			);

			$sql = \FGTA4\utils\SqlUtility::Select('con_weekdt', [
				'weekdt_id', 'weekdt_date', 'weekdt_dayname', 'weekdt_daynum', 'weekdt_year', 'weekdt_month', 'week_id', '_createby', '_createdate', '_modifyby', '_modifydate' 
			], $where->sql);
			...
			$result->record = array_merge($record, [
				'weekdt_date' => date("d/m/Y", strtotime($record['weekdt_date'])),
