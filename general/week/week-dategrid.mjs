...
const tbl_list = $('#pnl_editdategrid-tbl_list')
const txt_title = $('#pnl_editdategrid-title')
const pnl_control = $('#pnl_editdategrid-control')
const btn_removechecked  = $('#pnl_editdategrid-removechecked')
const btn_addrow = $('#pnl_editdategrid-addrow')
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/date-list`
		options.criteria['id'] = data.week_id
	}
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editdateform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editdateform', () => {
		$ui.getPages().ITEMS['pnl_editdateform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/date-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editdateform', ()=>{
		$ui.getPages().ITEMS['pnl_editdateform'].handler.createnew(header_data)
	})	
}