...
/**
 * ent/general/selector/apis/save.php
 *
 * ====
 * Save
 * ====
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel header selector (mst_selector)
 *
 ...
 * tanggal 25/08/2023
 */
$API = new class extends selectorBase {
	
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_selector';
		$primarykey = 'selector_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;
		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\selector_headerHandler";
		$hnd = null;
		if (class_exists($handlerclassname)) {
			$hnd = new selector_headerHandler($options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$obj->selector_text = strtoupper($obj->selector_text);
			...
			try {
				...
				$criteriaValues = [
					"selector_id" => " selector_id = :selector_id "
				];
				...
				$sqlFieldList = [
					'selector_id' => 'A.`selector_id`', 'selector_text' => 'A.`selector_text`', 'selector_data' => 'A.`selector_data`', '_createby' => 'A.`_createby`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_selector A";
				$sqlWhere = $where->sql;
