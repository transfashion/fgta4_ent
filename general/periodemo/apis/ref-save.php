...
/**
 * ent/general/periodemo/apis/ref-save.php
 *
 * ==========
 * Detil-Save
 * ==========
...
$API = new class extends periodemoBase {
	...
	public function execute($data, $options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new periodemo_refHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'periodemoref_id', 'interface_id', 'periodemoref_name', 'periodemoref_code',
					'periodemoref_otherdata', 'periodemoref_notes', 'periodemo_id'
			];
			...
			$obj->periodemoref_code = strtoupper($obj->periodemoref_code);


			if ($obj->periodemoref_otherdata=='') { $obj->periodemoref_otherdata = '--NULL--'; }
			if ($obj->periodemoref_notes=='') { $obj->periodemoref_notes = '--NULL--'; }
			...
			try {
				...
				// result
				$options->criteria = [
					"periodemoref_id" => $obj->periodemoref_id
				];
