...
const tbl_list = $('#pnl_editbilltypegrid-tbl_list');
const txt_title = $('#pnl_editbilltypegrid-title');
const pnl_control = $('#pnl_editbilltypegrid-control');
const btn_removechecked  = $('#pnl_editbilltypegrid-removechecked');
const btn_addrow = $('#pnl_editbilltypegrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/billtype-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editbilltypeform'].handler.getForm()
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editbilltypeform', () => {
		$ui.getPages().ITEMS['pnl_editbilltypeform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/billtype-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editbilltypeform', ()=>{
		$ui.getPages().ITEMS['pnl_editbilltypeform'].handler.createnew(header_data)
	})	
}