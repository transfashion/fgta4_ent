 ...
 * Detil-DataList
 * ==============
 * Menampilkan data-data pada tabel modeltransaksi partner (mst_partnertrxmodel)
...
$API = new class extends partnerBase {
	...
	public function execute($options) {
		...
		if (class_exists($handlerclassname)) {
			$hnd = new partner_modeltransaksiHandler($options);
			$hnd->caller = $this;
			$hnd->db = $this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlFieldList = [
				'partnertrxmodel_id' => 'A.`partnertrxmodel_id`', 'trxmodel_id' => 'A.`trxmodel_id`', 'coa_id' => 'A.`coa_id`', 'unbill_coa_id' => 'A.`unbill_coa_id`',
				'partner_id' => 'A.`partner_id`', '_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`',
				'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
			];
			$sqlFromTable = "mst_partnertrxmodel A";
			$sqlWhere = $where->sql;
			...
			foreach ($rows as $row) {
				...
				*/
				...
				$this->addFields('unbill_coa_name', 'unbill_coa_id', $record, 'mst_coa', 'coa_name', 'coa_id');
