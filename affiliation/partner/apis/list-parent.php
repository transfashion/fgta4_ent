...
/**
 * finact/master/partner/apis/list.php
 *
 * ========
 * DataList
 * ========
...
$API = new class extends partnerBase {
	...
	public function execute($options) {
		...
		try {
			...
			\FGTA4\utils\SqlUtility::setDefaultCriteria($options->criteria, 'partner_isparent', '1');
			...
			$where = \FGTA4\utils\SqlUtility::BuildCriteria(
				...
				[
					...
					"partner_isparent" => " A.partner_isparent = :partner_isparent ",
			...
			$stmt = $this->db->prepare("
				select 
				  A.partner_id
				, A.partner_name
				, A.partner_id as partner_parent, A.partner_name as partner_parent_name
				from mst_partner A
			" . $where->sql . $limit);
			$stmt->execute($where->params);
