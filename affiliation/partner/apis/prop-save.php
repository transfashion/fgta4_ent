...
/**
 * ent/affiliation/partner/apis/prop-save.php
 *
 * ==========
 * Detil-Save
 * ==========
 * Menampilkan satu baris data/record sesuai PrimaryKey,
 * dari tabel prop partner (mst_partnerprop)
 *
...
$API = new class extends partnerBase {
	...
	public function execute($data, $options) {
		$event = 'on-save';
		$tablename = 'mst_partnerprop';
		$primarykey = 'partnerprop_id';
		$autoid = $options->autoid;
		$datastate = $data->_state;

		$userdata = $this->auth->session_get_user();

		$handlerclassname = "\\FGTA4\\apis\\partner_propHandler";
		if (class_exists($handlerclassname)) {
			$hnd = new partner_propHandler($data, $options);
			$hnd->caller = &$this;
			$hnd->db = &$this->db;
			$hnd->auth = $this->auth;
		...
		try {
			...
			$sqlUpdateField  = [
					'partnerprop_id', 'partnerprop_key', 'partnerprop_value', 'partner_id'
			];
			...
			$obj->partnerprop_key = strtoupper($obj->partnerprop_key);
			...
			try {
				...
				// result
				$options->criteria = [
					"partnerprop_id" => $obj->partnerprop_id
				];

				$criteriaValues = [
					"partnerprop_id" => " partnerprop_id = :partnerprop_id "
				];
				...
				$sqlFieldList = [
					'partnerprop_id' => 'A.`partnerprop_id`', 'partnerprop_key' => 'A.`partnerprop_key`', 'partnerprop_value' => 'A.`partnerprop_value`', 'partner_id' => 'A.`partner_id`',
					'_createby' => 'A.`_createby`', '_createdate' => 'A.`_createdate`', '_modifyby' => 'A.`_modifyby`', '_modifydate' => 'A.`_modifydate`'
				];
				$sqlFromTable = "mst_partnerprop A";
				$sqlWhere = $where->sql;
