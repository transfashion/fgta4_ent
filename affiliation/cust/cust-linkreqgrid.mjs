...
import * as hnd from  './cust-linkreqgrid-hnd.mjs'

const tbl_list = $('#pnl_editlinkreqgrid-tbl_list');
const txt_title = $('#pnl_editlinkreqgrid-title');
const pnl_control = $('.pnl_editlinkreqgrid-control');
const btn_removechecked  = $('#pnl_editlinkreqgrid-removechecked');
const btn_addrow = $('#pnl_editlinkreqgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/linkreq-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editlinkreqform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editlinkreqgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editlinkreqgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editlinkreqform', () => {
		$ui.getPages().ITEMS['pnl_editlinkreqform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/linkreq-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editlinkreqform', ()=>{
		$ui.getPages().ITEMS['pnl_editlinkreqform'].handler.createnew(header_data)
	})	
}