...
import * as hnd from  './cust-accessgrid-hnd.mjs'

const tbl_list = $('#pnl_editaccessgrid-tbl_list');
const txt_title = $('#pnl_editaccessgrid-title');
const pnl_control = $('.pnl_editaccessgrid-control');
const btn_removechecked  = $('#pnl_editaccessgrid-removechecked');
const btn_addrow = $('#pnl_editaccessgrid-addrow');
...
export function OpenDetil(data) {
	...
	var fn_listloading = async (options) => {
		options.api = `${global.modulefullname}/access-list`
	...
	var fn_listloaded = async (result, options) => {
		...
		var detilform = $ui.getPages().ITEMS['pnl_editaccessform'].handler.getForm()
		...
		setTimeout(()=>{
			var checkcolumns = document.querySelectorAll('#pnl_editaccessgrid-tbl_list .rowcheck');
			...
			var selectbutton = document.getElementById('pnl_editaccessgrid-tbl_list-selectall-button');
			if (detilform.AllowRemoveRecord) {
...
function grd_list_rowclick(tr, ev) {
	...
	$ui.getPages().show('pnl_editaccessform', () => {
		$ui.getPages().ITEMS['pnl_editaccessform'].handler.open(record, trid, header_data)
	})	
}
		...
		yes: () => {
			grd_list.removechecked({
				OnRemoving : async (options) => {
					var apiurl = `${global.modulefullname}/access-delete`
...
function btn_addrow_click() {
	$ui.getPages().show('pnl_editaccessform', ()=>{
		$ui.getPages().ITEMS['pnl_editaccessform'].handler.createnew(header_data)
	})	
}